...
> Make a directory `/work/<username>` (`/work/` is your local hard drive) and install everything there.

```
git clone --recursive git@gitlab.vision.in.tum.de:visnav_ss20/<username>/visnav.git
```
...
We recommend to go for the latest release (currently 4.12), but it should be at least 4.8.
...
> ```
> sudo apt install qtcreator-4.12 clang-format
> ```
> You can start this version of QtCreator from the terminal with the command `qtcreator-4.12`.
...
>
> ```
> wget https://download.qt.io/official_releases/qtcreator/4.12/4.12.0/qt-creator-opensource-linux-x86_64-4.12.0.run
> chmod +x qt-creator-opensource-linux-x86_64-4.12.0.run
> ./qt-creator-opensource-linux-x86_64-4.12.0.run
> ```
>
> And `clang-format` you can get from the apt package:
>
> ```
